...
public class RunDebugSepsisCommand implements Runnable {

    @Override
    public void run() {
        Path inputPath = DebugJsonExporter.DEFAULT_INPUT_PATH;
        if (!Files.exists(inputPath)) {
            Dialogs.showErrorMessage("CZI Spatial Viewer", "Validation file not found: " + inputPath);
            return;
        }
        Thread worker = new Thread(() -> export(inputPath), "CZI Spatial Sepsis Debug Export");
    ...
    private void export(Path inputPath) {
        try {
            ...
            Dialogs.showInfoNotification("CZI Spatial Viewer", "Sepsis debug export completed in outputs/");
        } catch (Exception e) {
